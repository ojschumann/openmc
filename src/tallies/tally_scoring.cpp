...
void score_general_ce(Particle& p, int i_tally, int start_index,
...
#pragma omp atomic
    ...
    default_case:
      ...
      if (tally.estimator_ == TallyEstimator::ANALOG) {
        ...
        if (score_bin < 2)
          fatal_error(
            "Invalid score type on tally " + std::to_string(tally.id_));
        score = 0.;
        if (i_nuclide >= 0) {
  ...
  double flux)
{
  ...
  if (i_nuclide >= 0) {
    ...
    // Set atom density to unity to unify most of the score calculations
    atom_density = 1.0;
  }
  ...
  for (auto i = 0; i < tally.scores_.size(); ++i) {
    ...
    case SCORE_INVERSE_VELOCITY:
      if (tally.estimator_ == TallyEstimator::ANALOG ||
        ...
        score *= flux 
                 * nuc_xs.get_xs(MgxsType::INVERSE_VELOCITY, p_g)
                 / macro_xs.get_xs(MgxsType::TOTAL, p_g);        
      } else {
    ...
    case SCORE_NU_FISSION:
      if (tally.estimator_ == TallyEstimator::ANALOG) {
        ...
        score = atom_density * flux * nuc_xs.get_xs(MgxsType::NU_FISSION, p_g);
      }
      break;
